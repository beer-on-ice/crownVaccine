...
(function (wid, dcm) {
	...
	function on(action, selector, callback) {
		...
		var classes = document.getElementsByClassName(selector);
		if(classes){
			for(var i=0;i<classes.length;i++){
				classes[i].addEventListener(action,function(e){
					callback(e);
				})
			}
		}
		var idsingle = document.getElementById(selector);
		if(idsingle){
			idsingle.addEventListener(action,function(e){
				callback(e);
			})
		}
	}
	...
	function generateTitleMonth(idx, year, month, cal) {
		...
		return new Date(year, month).getFullYear()+ '年 ' + monthArr[0][new Date(year, month).getMonth()];
	}
	
	function generateItemTitle(cal) {
		var chinese       = '<span>一</span><span>二</span><span>三</span><span>四</span><span>五</span><span class="red">六</span>';
		var english       = '<span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span class="red">S</span>';
		var chineseString = cal.isSundayFirst ? ('<span class="red">日</span>' + chinese) : (chinese + '<span class="red">日</span>');
		var englishString = cal.isSundayFirst ? ('<span class="red">S</span>' + english) : (english + '<span class="red">S</span>');
	...
	function switchItemBody(direct, distance, cal) {
		...
		var applyYear  = new Date(cal.currentYear, cal.currentMonth + 1).getFullYear();
		var applyMonth = new Date(cal.currentYear, cal.currentMonth + 1).getMonth();
		
		cal.box.querySelectorAll('.calendar-item.calendar-item' + itemNum).forEach(function (obj) {
			obj.innerHTML = generateItemBodyDom(cal.currentYear, cal.currentMonth + 1, cal);
	...
	function touchMove(event, cal) {
		...
		if (Math.abs(cal.move.S) >= Math.abs(cal.move.standardS) && !cal.isMask) {
			...
			transformFormat(cal.box, cal.distance, .1);
		} else {
	...
	function touchEnd(event, cal) {
		...
		if (cal.end.time - cal.start.time < 200 && Math.abs(tempDis) < 10) {
			...
			transformFormat(cal.box, cal.distance, 0.1);
			...
			transformFormat(cal.box, enddis * cal.width, 0.1);
			...
			if (_this.isToggleBtn) {
				on('touchstart', _this.container + 'CalendarTitleLeft', function () {
					...
					setTimeout(function () {
						...
						transformFormat(_this.box, _this.distance, .1);
		...
		renderCallbackArr: function (arr) {
			...
			loop(0, arr.length, function (k) {
				if (!$class(_this.container + '-item-' + arr[k].stamp)[0]) {
					...
					return true;*/
					var list = document.querySelectorAll("i[data-stamp='"+arr[k].stamp+"']")
					if(list.length>0){
						list.forEach(function(item){
							item.parentNode.className += " "+arr[k].className;
						})
					}
					return false;
				}
