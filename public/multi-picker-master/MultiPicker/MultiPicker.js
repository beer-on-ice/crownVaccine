	...
	MultiPicker.prototype = {
		...
		insertLiArr: function (targetUl, arr) {
			...
			loop(0, arr.length, function (i) {
				html += '<li data-id="' + arr[i].id + '" data-code="' + arr[i].code + '">' + arr[i].value + '</li>';
			});
		...
		initReady: function (idx, target) {
			...
			loop(0, _this.idxArr.length, function (i) {
				...
				_this.resultArr.push({
					...
					"code": _this.jsonArr[i][_this.distance[i] / _this.liHeight + 2].code,
		...
		initBinding: function () {
			...
			var body      = doc.body;
			on('touchend', _this.input, function (e) {
				if(movingflag){
		      		return;
		      	}
