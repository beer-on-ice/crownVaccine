...
(function (wid, dcm) {
  ...
  function on(action, selector, callback) {
    /*doc.addEventListener(action, function (e) {
      if (selector == e.target.id || selector+"-icon" == e.target.id) {
        callback(e);
        e.preventDefault();
      }
    })*/
    doc.getElementById(selector)?(doc.getElementById(selector).addEventListener(action, function (e) {
        callback(e);
        e.preventDefault();
    })):null;
    doc.getElementById(selector+"-icon")?(doc.getElementById(selector+"-icon").addEventListener(action, function (e) {
        callback(e);
        e.preventDefault();
    })):null;
  }
  ...
  DateSelector.prototype = {
    ...
    show: function(){
    	var btn = document.querySelectorAll('#'+this.input)[0];  
		var event = document.createEvent('Events');
		event.initEvent('touchend', true, true); 
		btn.dispatchEvent(event);
    },
    showText: function(text){
    	document.getElementById("showText"+this.container).innerHTML = text;
    	document.getElementById("showText"+this.container).parentElement.style.display = "block";
    },
    checkTime: function () {
      var _this       = this;
      ...
      if(!!document.getElementById(_this.input).value){
      	_this.recentTime = [];
      	var timearr = document.getElementById(_this.input).value.split("-");
  		loop(0, _this.idxArr.length, function (i) {
          _this.recentTime.push(parseInt(timearr[i]));
        });
      }
    ...
    initDomFuc: function () {
      ...
      html += '<div class="date-selector-bg" id="date-selector-bg-' + _this.container + '">' +
        ...
        '<div class="date_selector-showtext-outer"><div class="date_selector-showtext" id="showText'+_this.container+'"></div></div>' +
        '<div class="date-selector-btn-box">' +
        '<div class="date-selector-btn" id="date-selector-btn-cancel-' + _this.container + '">取消</div>';
    ...
    initBindingBtn: function () {
      var _this     = this;
      var body      = doc.body;

      on('touchend', 'date-selector-btn-save-' + _this.container, function () {
        ...
        document.getElementById("showText"+_this.container).parentElement.style.display = "none";
      }, false);

      on('touchend', 'date-selector-bg-' + _this.container, function () {
	    var bg        = $id('date-selector-bg-' + _this.container);
	    var container = $id('date-selector-container-' + _this.container);
      ...
      on('touchend', 'date-selector-btn-cancel-' + _this.container, function () {
	    var bg        = $id('date-selector-bg-' + _this.container);
	    var container = $id('date-selector-container-' + _this.container);
      	if(_this.cancel) _this.cancel();
        bg.classList.remove('date-selector-bg-up');
    ...
    initBinding: function () {
      var _this     = this;
      var body      = doc.body;
      on('touchend', _this.input, function () {
      	if(movingflag){
      		return;
      	}
	    var bg        = $id('date-selector-bg-' + _this.container);
        ...
        if(document.getElementById("showText"+_this.container).innerHTML){
			document.getElementById("showText"+_this.container).parentElement.style.display = "block";
		}
      }, false);
      ...
      on('touchend', 'date-selector-tab date-selector-' + _this.container + '-tab', function (event) {
	    var bg        = $id('date-selector-bg-' + _this.container);
	    var container = $id('date-selector-container-' + _this.container);
